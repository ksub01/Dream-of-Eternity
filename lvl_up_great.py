...
def characteristic_upgrade():
    ...
    while 1:
        number_characteristic = int(input('Здоровье + {} -> 1\n'
                                          ...
                                          'Ловкость + {} -> 3\n'
                                          'Мудрость + {} -> 4\n'
                                          ''.format(10 * (hero_stats_great.parameter['lvl'] - 1),
                                                    2 * (hero_stats_great.parameter['lvl'] - 1),
                                                    3 * (hero_stats_great.parameter['lvl'] - 1),
                                                    4 * (hero_stats_great.parameter['lvl'] - 1))))
        if number_characteristic == 1:
            hero_stats_great.heart_full_upgrade(10 * (hero_stats_great.parameter['lvl'] - 1))
            break
        elif number_characteristic == 2:
            hero_stats_great.force_upgrade(2 * (hero_stats_great.parameter['lvl'] - 1))
            break
        elif number_characteristic == 3:
            hero_stats_great.dexterity_upgrade(3 * (hero_stats_great.parameter['lvl'] - 1))
            break
        elif number_characteristic == 4:
            hero_stats_great.wisdom_upgrade(4 * (hero_stats_great.parameter['lvl'] - 1))
            break
        else:
            pass
...
def skills():  # выбор скила
    ...
    while 1:
        display_skills()
        if hero_stats_great.parameter['name'] == 'Повелитель':
            choice_skills_lord()
            break
        else:
            print('Не готово')
...
def display_skills():
    ...
    if hero_stats_great.parameter['name'] == 'Повелитель':
        if 'Длань господа' in hero_stats_great.nav:
            hand_of_god = hero_stats_great.nav['Длань господа'][0]
        else:
            hand_of_god = 0
        ...
        if 'Божественное провиденье' in hero_stats_great.nav:
            divine_providence = hero_stats_great.nav['Божественное провиденье'][0]
        else:
            divine_providence = 0
        print('2 Божественное провиденье *{}*: Увеличивает вашу атаку на [5, 10, 20]'
              ''.format(divine_providence))

        if 'Демонический облик' in hero_stats_great.nav:
            demon_outlook = hero_stats_great.nav['Демонический облик'][0]
        else:
            demon_outlook = 0
        print('3 Демонический облик *{}*: Ваша атака увеличивается в 2 раза,\n'
              'но вы возвращаете [50, 40, 30] процентов себе'.format(demon_outlook))
    else:
        print('Для других классов не готово')


def choice_skills_lord():
    ...
    while 1:
        ...
        if number == 1:
            if upgrade_in_skills_nav('Длань господа', [0, {1: 5, 2: 10, 3: 15}]) == 1:
                break
            else:
                pass
        elif number == 2:
            if upgrade_in_skills_nav('Божественное провиденье', [0, {1: 5, 2: 10, 3: 20}]) == 1:
                break
            else:
                pass
        elif number == 3:
            if upgrade_in_skills_nav('Демонический облик', [0, [50, 40, 30]]) == 1:
                break
            else:
                pass
        else:
...
def upgrade_in_skills_nav(name, grade):
    ...
    if name in hero_stats_great.nav:
        if hero_stats_great.nav[name][0] < 3:
            hero_stats_great.upgrade_nav(name)
            return 1
        else:
            print('Навык {} нельзя больше прокачать'.format(name))
            return 0
    else:
        hero_stats_great.new_nav(name, grade)
        return 1
