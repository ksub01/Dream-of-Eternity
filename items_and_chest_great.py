"""Модуль, в котором перечислены все предметы и механика получения сундуков, добавляя их в инвентарь
Типы в словарях для обработки всего подряд?"""
...
def chest_open(num):
    ...
    количество которого зависит от уровня героя, причём предмет, доставаемый из сундука, находится в словаре под
    номером, совпадающим с уровнем сундука. Для каждого из четырёх предметов своё ветвление в рандоме"""
    throw = random.randint(1, 25)
    if throw == 1:
        inventory_hero_great.give_sword(rare_swords[num])
        start_game_great.outlast()
    elif throw == 2:
        inventory_hero_great.give_armor(rare_armor[num])
        start_game_great.outlast()
    elif throw == 3:
        inventory_hero_great.give_cloak(rare_cloak[num])
        start_game_great.outlast()
    elif throw == 4:
        inventory_hero_great.give_ring(rare_ring[num])
        start_game_great.outlast()
    else:
        # игрок получает количество золота, помноженное на уровень, если предмета не выпало
        hero_stats_great.gold_receive(throw * hero_stats_great.parameter['lvl'])
        start_game_great.outlast()


"""Меч труса повышает вероятность побега
